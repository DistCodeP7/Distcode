...
import { Node, FileNode, FolderNode } from "./problemEditorClient";
...
export interface EditableFileNode extends FileNode { readOnly?: boolean }
...
export function FilteredTreeNode({
    ...
    const [isOpen, setIsOpen] = useState(isFolder ? (node as FolderNode).isOpen ?? true : false);
    const padding = { paddingLeft: level * 16 };

    if (node.type === "file") {
        const file = node as EditableFileNode;
        const handleActivate = () => { if (!file.readOnly) onFileClick(file); };
        const canDelete = onDeleteFile && !file.path.endsWith("main.go") && file.path.startsWith("/template");
