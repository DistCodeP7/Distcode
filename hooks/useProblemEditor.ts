...
const getInitialContent = (file: ProblemFile): string => {
  if (file.fileType === "markdown") return "# Problem\n\nDescribe the problem here.";
  if (file.fileType === "go") {
    if (file.name.startsWith("template")) return "// Write your template code here\n";
    if (file.name.startsWith("solution")) return "// Write your solution code here\n";
...
export const useProblemEditor = (
  ...
  const [state, setState] = useState<ProblemEditorState>(() => {
    const filesContent = files.reduce((acc, file) => {
      acc[file.name] = initial?.filesContent?.[file.name] ?? getInitialContent(file);
      return acc;
  ...
  const syncFilesContent = useCallback(() => {
    setState((prev) => {
      const newFilesContent = files.reduce((acc, file) => {
        acc[file.name] = prev.filesContent[file.name] ?? getInitialContent(file);
        return acc;
      }, {} as Record<string, string>);
  ...
  const handleSaveOrSubmit = useCallback(
      async (isPublished: boolean) => {
        ...
        try {
          ...
          alert(result.success ? result.message || "Success!" : result.error || "Error.");
        } catch (err) {
      ...
      [state, files, initial?.submissionId]
  );

  const handleSubmit = useCallback(() => handleSaveOrSubmit(true), [handleSaveOrSubmit]);
  const handleSave = useCallback(() => handleSaveOrSubmit(false), [handleSaveOrSubmit]);
